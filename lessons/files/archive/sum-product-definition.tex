...
\newcommand{\fwd}{\overrightarrow}
\newcommand{\bck}{\overleftarrow}
...
% factors
\node (g1) [factor,dashed] at (-2.5,1.5)  {}; 
\node (gn) [factor,dashed] at (-2.5,-1.5) {};
\node (f)  [factor] at (0,0){$f$};
\node (h)  [factor,dashed] at (2.5,-0) {};


% edges 
\path (-2.5,.8) edge[dotted,thick] (-2.5,-.8);
\path (g1) edge[->] node[above] {$X_1$} node[swap] {$\fwd{\mu_1}$} (f) ;
\path (gn) edge[->]  node[below] {$X_n$} node[above] {$\fwd{\mu_n}$} (f); 
\path (f) edge[->]  node[above] {$Y$} node[below left] {$\fwd{\mu_y}$} (h) ;
